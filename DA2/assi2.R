...
#mydata <-read.csv(file = "D:\\DPS\\Class_data\\BE\\LP1\\diabetes.csv",header=TRUE,sep=",")
#View(mydata)

mydata <- read.csv("DA/DA2/pima.csv",header=TRUE)
...
my_model <-naiveBayes(as.factor(train$diabetes)~.,train)
my_model
...
table(pred1,test$diabetes,dnn=c("predicted","actual"))
